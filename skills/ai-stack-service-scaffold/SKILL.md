...
- `<service>/.config.env.example` (safe defaults, no secrets)
- Optional: `<service>/README.md` and/or `<service>/OPERATIONS.md` (ports, volumes, restore steps)

## Compose conventions (ai_stack)

- Use the external docker network `ai-stack` (shared between stacks).
...
- Keep secrets out of git; rely on `docker compose --env-file .env --env-file .config.env --env-file <service>/.config.env ...` (all env files are gitignored).
