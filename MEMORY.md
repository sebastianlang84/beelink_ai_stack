...
- `fourier-cycles` now projects cycle components by default for `120` days beyond the latest observed date (`FOURIER_PROJECTION_DAYS`), writes projection markers in `waves.csv` (`is_projection`), and renders projected UI overlays dashed beyond the last price point.
- `fourier-cycles` now includes `tools/cycle_spectrum_like_table.py` to emulate third-party cycle-spectrum number tables for test comparisons (`Len/Amp/Strg/Stab` + selected markers) directly from `cycles.csv` + `summary.json`.
- `fourier-cycles` synthetic ground-truth checker (`tools/synthetic_superposition_check.py`) now supports pipeline-like tuning flags and writes `analysis_cfg` in `summary.json`; the current tuned fixture (`windows=240,480,960`, `step=20`, `snr_presence=1.5`, `selection_norm_power_pct=0.2`) reaches `5/5` across seeds `7,21,42,77,123`.
