...
usage() {
  cat <<'EOF'
Usage:
  ./scripts/run_emb_bench.sh [--env-file .env] -- <emb_bench args...>

Examples:
  ./scripts/run_emb_bench.sh -- python -m emb_bench run --config config.local_only.yaml --phase local_vs_remote
  ./scripts/run_emb_bench.sh --env-file .env -- python -m emb_bench run --config config.example.yaml --phase mrl --subset-docs 200 --subset-queries 50
